...
const PIECES: Piece[] = [
  ...
  { shape: [[1, 1], [1, 1]], color: 6 },          // O
  { shape: [[0, 1, 0], [1, 1, 1]], color: 1 },     // T
  { shape: [[1, 0, 0], [1, 1, 1]], color: 2 },     // L
  { shape: [[0, 0, 1], [1, 1, 1]], color: 3 },     // J
...
export const TetrisGame = () => {
  ...
  useEffect(() => {
    ...
    const handleKey = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        ...
        case 'ArrowDown': case 's': case 'S': tick(); break
        case 'ArrowUp': case 'w': case 'W': rotate_(); break
        case ' ': drop(); break
        case 'p': case 'P': case 'Escape': setPaused(p => !p); break
      }
    }
  ...
  useEffect(() => {
    if (!started) return
    let raf: number
    const loopy = () => { render(); raf = requestAnimationFrame(loopy) }
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      <div className="flex gap-6 text-sm font-mono">
        ...
        <div className="text-green-400">LIGNES <span className="text-white font-bold">{lines}</span></div>
      </div>
        ...
        {!started && !gameOver && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-dark-bg/85 rounded-xl">
            ...
            <div className="text-gray-400 text-xs mb-6 text-center">
              <span className="text-white">←→</span> bouger · <span className="text-white">↑</span> rotate · <span className="text-white">↓</span> tomber<br />
              <span className="text-white">SPACE</span> drop · <span className="text-white">P</span> pause
            </div>
            <button
              onClick={startGame}
