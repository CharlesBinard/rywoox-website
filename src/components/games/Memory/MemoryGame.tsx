  ...
  return (
    <div className="flex flex-col items-center gap-6">
      <div className="flex gap-8 text-lg font-mono">
        <div className="text-neon-pink">COUPS <span className="text-white font-bold">{moves}</span></div>
        <div className="text-green-400">PAIRES <span className="text-white font-bold">{matched}/{EMOJIS.length}</span></div>
      </div>
      ...
      {gameOver && (
        ...
        >
          ...
          <div className="text-gray-400">{moves} coups · {EMOJIS.length} paires</div>
          <button
            onClick={reset}
            className="px-8 py-3 rounded-lg bg-neon-pink/20 border border-neon-pink text-neon-pink font-bold hover:bg-neon-pink/30 transition-all cursor-pointer"
          >
