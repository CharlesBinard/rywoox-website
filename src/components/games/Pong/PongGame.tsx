...
export const PongGame = () => {
  ...
  useEffect(() => {
    const handleKey = (e: KeyboardEvent) => {
      ...
      if ((e.key === ' ' || e.key === 'Enter') && (gameState === 'idle' || gameState === 'countdown')) {
        startGame()
      }
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      if (keysRef.current.has('ArrowDown') || keysRef.current.has('s') || keysRef.current.has('S')) {
        playerPaddleRef.current = Math.min(HEIGHT - PADDLE_H, playerPaddleRef.current + PLAYER_SPEED)
      }
      ...
      if (gameState === 'playing') {
        ...
        if (ball.x < 0) {
          setAiScore(s => {
            const ns = s + 1
            if (ns >= WIN_SCORE) { setWinner('ai'); setGameState('gameover') }
            else {
        ...
        if (ball.x > WIDTH) {
          setPlayerScore(s => {
            const ns = s + 1
            if (ns >= WIN_SCORE) { setWinner('player'); setGameState('gameover') }
            else {
              launchDirRef.current = -1
              resetBall(-1)
              setCountdown(2)
  ...
  return (
    <div className="flex flex-col items-center gap-6">
      <div className="flex gap-12 text-2xl font-mono font-bold">
        <div className="text-neon-cyan">TOI <span className="text-white">{playerScore}</span></div>
        <div className="text-neon-purple">IA <span className="text-white">{aiScore}</span></div>
      </div>
      ...
      <div className="relative">
        ...
        {gameState === 'idle' && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-dark-bg/85 rounded-xl">
            ...
            <div className="text-gray-400 text-sm mb-6 text-center">
              <span className="text-white">↑↓</span> ou <span className="text-white">W/S</span> pour bouger
            </div>
            <button
              onClick={startGame}
        ...
        {gameState === 'gameover' && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-dark-bg/85 rounded-xl">
            ...
            <div className={`text-2xl font-bold mb-2 ${winner === 'player' ? 'text-neon-cyan' : 'text-neon-purple'}`}>
              {winner === 'player' ? 'TROP FORT !' : 'L\'IA GAGNE...'}
            </div>
            <div className="text-gray-400 mb-6">{playerScore} - {aiScore}</div>
            <button
              onClick={startGame}
              className="px-8 py-3 rounded-lg bg-neon-cyan/20 border border-neon-cyan text-neon-cyan font-bold hover:bg-neon-cyan/30 transition-all cursor-pointer"
            >
